...
data pname/'neutro','proton','he---4','li---7','be---9','b---11','c---12','n---14','o---16','f---19' & 
  ...
  do ip=0,npart ! currently only neutron & proton
   if(iangpart(ip).ne.0) then
    ...
    if(isout.eq.2) then ! as a function of energy
     ...
     write(100+ip,'(''     E(MeV/n)'',201f13.2,'' (/(MeV/n)/cm2/s/sr)'')') (ang(ia),ia=1,nang)
     ...
     if(iangpart(ip).eq.4) then ! for muon
      do ie=1,10 ! extra one order
       etmp=emid(ie)*1.0e14
       write(100+ip,'(202es13.5)') etmp,(getSpec(ip,s,r,d,etmp,g) &
 &     *getSpecAngFinal(iangpart(ip),s,r,d,etmp,g,ang(ia)),ia=1,nang)
      enddo
     endif
    elseif(isout.eq.3) then ! as a function of cos(theta), relative value
     ...
     write(100+ip,'(200i13)') (ie,ie=1,nebin+3-iemin)
     write(100+ip,'(''   cos(theta)'',200es13.5,'' (/(MeV/n)/cm2/s/sr)'')') & 
   & (emid(ie),ie=iemin,nebin+1)
     do ia=1,nang ! output result
